...
{
    // Conway Bevel operator: truncates edges
    // Creates rectangular faces along edges
    // Creates small faces at vertices
    ...
    public static class ConwayBevel
    {
        public static MeshData Apply(MeshData input, float ratio, Allocator allocator)
        {
            ...
            ratio = math.clamp(ratio, 0.1f, 0.4f);
            ...
            for (var heIdx = 0; heIdx < input.halfEdgeCount; heIdx++)
            {
                ...
                // Create vertex near start (moved toward midpoint)
            ...
            // 1. Recreate original faces with expanded vertex count
            ...
            for (var heIdx = 0; heIdx < input.halfEdgeCount; heIdx++)
            {
                ...
                if (halfEdge.twin == -1 || heIdx < halfEdge.twin)
                {
                    ...
                    // Rectangle: end of he -> start of twin.next -> end of twin.next -> start of he.next
